...
counter = 1;
 ...
 DDJ_FP = 'GAAAGACGTCACAGCAAGGT';% FP+b from second base
...
while ischar(tempstr)
     ...
     if (strfind(tempseq(2:21),DDJ_FP)~=0) & (strfind(tempseq(117:end),DDJ_RP)~=0)
...
for i= 1:length(seq)
    ...
    [~,index] = ismember(seq{i}(47:56),Address_ref);

    if index == 0
        continue
        
    else
        Message{index,count(index)} = seq{i}(57:116); % message to message end
        Pixel{index,count(index)} = seq{i}(47:116);
...
%writecell(DominantMess,strcat('lib',num2str(libnumber),'_DDJ_dominantmess.csv'))
a=-1*ones(5, 255, 2);
...
writematrix(bb,strcat('lib',num2str(libnumber),'_DDJ_.dat'),'Delimiter',' ')
...
DominantMess2 = readcell('/data/wengz/DNA_storage/primer_oligo_sequence/Original_Mess/OriginalMess_DDJ.xlsx')
...
writetable(MissmatchedMess2,strcat('DDJ_MissmatchedMess',num2str(libnumber),'.csv'))
end
...
saveas(gcf,strcat('lib',num2str(libnumber),'_DDJ_oligocomparison.jpg'))
writematrix(dd,strcat('lib',num2str(libnumber),'_DDJ_oligocomparison.csv'))
