...
counter = 1;
 ...
 DDJ_FP = 'CTCATCTGTAAAGCAGGGAGAGA';% FP+b from second base
 DDJ_RP = 'CTGCTGGATATCTGATGGCTGT';
 
while ischar(tempstr)
     
     tempseq = fgetl(fid);
     
     if (strfind(tempseq(2:24),DDJ_FP)~=0) & (strfind(tempseq(112:end),DDJ_RP)~=0)
...
for i= 1:length(seq)
    ...
    [~,index] = ismember(seq{i}(42:51),Address_ref);

    if index == 0
        continue
        
    else
        Message{index,count(index)} = seq{i}(52:111); % message to message end
        Pixel{index,count(index)} = seq{i}(42:111);
...
DominantMess = cell(1,765); % 65025= 5*255
ConsensusRatio = -1*ones(1,765);
...
%writecell(DominantMess,strcat('lib',num2str(libnumber),'_IHAD_dominantmess.csv'))
a=-1*ones(3, 255, 2);
...
writematrix(bb,strcat('lib',num2str(libnumber),'_IHAD_.dat'),'Delimiter',' ')


close all

DominantMess2 = readcell('/data/wengz/DNA_storage/primer_oligo_sequence/Original_Mess/OriginalMess_IHAD.xlsx')
...
writetable(MissmatchedMess2,strcat('IHAD_MissmatchedMess',num2str(libnumber),'.csv'))
end
...
saveas(gcf,strcat('lib',num2str(libnumber),'_IHAD_oligocomparison.jpg'))
writematrix(dd,strcat('lib',num2str(libnumber),'_IHAD_oligocomparison.csv'))
