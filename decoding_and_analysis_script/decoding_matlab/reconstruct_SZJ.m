...
counter = 1;
 ...
 DDJ_FP = 'GTAGGAGAGATTGGGCTAGAGAG';% FP+b from second base
 DDJ_RP = 'ATACCATGAGAGGCAGAGTGTGG';
 
while ischar(tempstr)
     
     tempseq = fgetl(fid);
     
     if (strfind(tempseq(2:24),DDJ_FP)~=0) & (strfind(tempseq(121:end),DDJ_RP)~=0)
...
for i= 1:length(seq)
    ...
    [~,index] = ismember(seq{i}(51:60),Address_ref);

    if index == 0
        continue
        
    else
        Message{index,count(index)} = seq{i}(61:120); % message to message end
        Pixel{index,count(index)} = seq{i}(51:120);
...
DominantMess = cell(1,510); % 65025= 5*255
ConsensusRatio = -1*ones(1,510);
...
%writecell(DominantMess,strcat('lib',num2str(libnumber),'_SZJ_dominantmess.csv'))
a=-1*ones(2, 255, 2);
...
writematrix(bb,strcat('lib',num2str(libnumber),'_SZJ_.dat'),'Delimiter',' ')
...
DominantMess2 = readcell('/data/wengz/DNA_storage/primer_oligo_sequence/Original_Mess/OriginalMess_SZJ.xlsx')
...
writetable(MissmatchedMess2,strcat('SZJ_MissmatchedMess',num2str(libnumber),'.csv'))
end
...
saveas(gcf,strcat('lib',num2str(libnumber),'_SZJ_oligocomparison.jpg'))
writematrix(dd,strcat('lib',num2str(libnumber),'_SZJ_oligocomparison.csv'))