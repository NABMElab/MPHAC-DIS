...
counter = 1;
 ...
 DDJ_FP = 'GATGGGACTCCAATGCAAAACT';% FP+b from second base
 DDJ_RP = 'TGATGTGGCATATCCTTCAGTAACTT';
 
while ischar(tempstr)
     
     tempseq = fgetl(fid);
     
     if (strfind(tempseq(2:23),DDJ_FP)~=0) & (strfind(tempseq(116:end),DDJ_RP)~=0)
...
DominantMess = cell(1,3825); % 65025= 5*255
ConsensusRatio = -1*ones(1,3825);
...
%writecell(DominantMess,strcat('lib',num2str(libnumber),'_LY_dominantmess.csv'))
a=-1*ones(15, 255, 2);
...
writematrix(bb,strcat('lib',num2str(libnumber),'_LY_.dat'),'Delimiter',' ')


close all

DominantMess2 = readcell('/data/wengz/DNA_storage/primer_oligo_sequence/Original_Mess/OriginalMess_LY.xlsx');
...
writetable(MissmatchedMess2,strcat('LY_MissmatchedMess',num2str(libnumber),'.csv'))
end
...
saveas(gcf,strcat('lib',num2str(libnumber),'_LY_oligocomparison.jpg'))
writematrix(dd,strcat('lib',num2str(libnumber),'_LY_oligocomparison.csv'))
